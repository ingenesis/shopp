...
if ( class_exists('WP_Widget') && ! class_exists('ShoppCategoriesWidget') ) {
	...
	class ShoppCategoriesWidget extends WP_Widget {
	    ...
	    function form($options) {
			...
			<input type="hidden" name="<?php echo $this->get_field_name('linkall'); ?>" value="0" /><input type="checkbox" id="<?php echo $this->get_field_id('linkall'); ?>" name="<?php echo $this->get_field_name('linkall'); ?>" value="1"<?php echo $options['linkall'] == "1"?' checked="checked"':''; ?> /><label for="<?php echo $this->get_field_id('linkall'); ?>"> <?php _e('Link empty categories','Shopp'); ?></label><br />
			</p>
