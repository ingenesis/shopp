<div class="wrap shopp">
	...
	<?php if (count(shopp_setting('target_markets')) == 0) echo '<div class="error"><p>' . Shopp::__('No target markets have been selected in your store setup.') . '</p></div>'; ?>
	...
	<div class="tablenav">
		<div class="actions">
		<button type="submit" name="addrate" id="addrate" class="button-secondary" tabindex="9999" <?php if (empty($countries)) echo 'disabled="disabled"'; ?>><?php Shopp::_e('Add Tax Rate'); ?></button>
		</div>
	</div>
	...
	<tr class="inline-edit-row ${classnames}" id="${id}">
		...
		<table id="taxrate-editor">
			<tr>
			<td scope="row" valign="top" class="rate"><input type="text" name="settings[taxrates][${id}][rate]" id="tax-rate" value="${rate}" size="7" class="selectall" tabindex="1" /><br /><label for="tax-rate"><?php Shopp::_e('Tax Rate'); ?></label><br />
			...
			?>
				<div class="conditionals no-conditions">
					<p><label><?php Shopp::__('Apply tax rate when %s of the following conditions match', $menu); ?>:</label></p>
					<ul>
					${conditions}
					</ul>
				</div>
				...
				<td>
					<div class="local-rates panel subpanel no-local-rates">
						<div class="label"><label><?php Shopp::_e('Local Rates'); echo ShoppAdmin()->boxhelp('settings-taxes-localrates'); ?> <span class="counter"></span><input type="hidden" name="settings[taxrates][${id}][haslocals]" value="${haslocals}" class="has-locals" /></label></div>
						<div class="ui">
	...
	<table class="widefat" cellspacing="0">
		...
		<?php
			...
			if (count($rates) == 0 && $edit === false): ?>
				<tr id="no-taxrates"><td colspan="5"><?php Shopp::_e('No tax rates, yet.'); ?></td></tr>
			<?php
			endif;
		...
		<tr class="<?php echo join(' ',$classes); ?>" id="taxrates-<?php echo $index; ?>">
			<td class="name column-name"><a href="<?php echo esc_url($editurl); ?>" title="<?php Shopp::_e('Edit'); ?> &quot;<?php echo esc_attr($rate); ?>&quot;" class="edit row-title"><?php echo esc_html($label); ?></a>
				<div class="row-actions">
					<span class='edit'><a href="<?php echo esc_url($editurl); ?>" title="<?php Shopp::_e('Edit'); ?> &quot;<?php echo esc_attr($label); ?>&quot;" class="edit"><?php Shopp::_e('Edit'); ?></a> | </span><span class='delete'><a href="<?php echo esc_url($deleteurl); ?>" title="<?php Shopp::_e('Delete'); ?> &quot;<?php echo esc_attr($label); ?>&quot;" class="delete"><?php Shopp::_e('Delete'); ?></a></span>
				</div>
			</td>
