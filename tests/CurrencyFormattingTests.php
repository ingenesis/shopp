...
class CurrencyFormatting extends ShoppTestCase {
	...
	function currency_assertions ($float,$expected,$code) {
		...
		$this->assertEquals(round($float,$format['precision']),Shopp::floatval($nf,true,$format),"Float value failed to reverse the numeric format of $nf for country code $code ".serialize($format));
