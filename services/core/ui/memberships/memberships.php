<div class="wrap shopp">
	...
	<h2><?php Shopp::_e('Memberships'); ?> <a href="<?php echo esc_url( add_query_arg(array('page'=>$this->Admin->pagename('memberships'),'id'=>'new'),admin_url('admin.php'))); ?>" class="button add-new"><?php Shopp::_e('Add New'); ?></a></h2>
	...
	<div class="tablenav">
		...
		<?php if(current_user_can('shopp_delete_customers')): ?><button type="submit" id="delete-button" name="deleting" value="customer" class="button-secondary"><?php Shopp::_e('Delete'); ?></button><?php endif; ?>
			</div>
	...
	<?php if (sizeof($MemberPlans) > 0): ?>
		...
		<?php
			...
			foreach ($MemberPlans as $MemberPlan):
				$MemberPlanName = empty($MemberPlan->name)?'('.Shopp::__('no membership name').')':$MemberPlan->name;
		...
		<tr<?php if (!$even) echo " class='alternate'"; $even = !$even; ?>>
			...
			<td class="name column-name"><a class='row-title' href='<?php echo $editurl; ?>' title='<?php Shopp::_e('Edit'); ?> &quot;<?php echo esc_attr($MemberPlanName); ?>&quot;'><?php echo esc_html($MemberPlanName); ?></a>
				<div class="row-actions">
					<span class='edit'><a href="<?php echo esc_url($editurl); ?>" title="<?php Shopp::_e('Edit'); ?> &quot;<?php echo esc_attr($MemberPlanName); ?>&quot;"><?php Shopp::_e('Edit'); ?></a> | </span>
					<span class='delete'><a class='submitdelete' title='<?php Shopp::_e('Delete'); ?> &quot;<?php echo esc_attr($MemberPlanName); ?>&quot;' href="<?php echo esc_url($deleteurl); ?>" rel="<?php echo $MemberPlan->id; ?>"><?php Shopp::_e('Delete'); ?></a></span>
				</div>
			</td>
	...
	<?php else: ?>
		<tbody><tr><td colspan="6"><?php Shopp::_e('No memberships found.'); ?></td></tr></tbody>
	<?php endif; ?>
	</table>
...
$('a.submitdelete').click(function () {
	if (confirm("<?php Shopp::_e('You are about to delete this membership!\n \'Cancel\' to stop, \'OK\' to delete.'); ?>")) {
...
$('#delete-button').click(function() {
	if (confirm("<?php echo addslashes(Shopp::__('Are you sure you want to delete the selected memberships?')); ?>")) {
