	...
	static function variations ($result, $options, $O) {
		...
		if ('single' == $mode) {
			...
			foreach ($O->prices as $pricetag) {
				...
				$discount = 100-round($pricetag->promoprice*100/$pricetag->price);
				$_ = new StdClass();
				if ($pricetag->type != 'Donation')
				  $_->p = money($currently);
				$_->l = $pricetag->label;
				$_->i = str_true($pricetag->inventory);
				if ($_->i) $_->s = $pricetag->stock;
				$_->u = $pricetag->sku;
				$_->tax = str_true($pricetag->tax);
				$_->t = $pricetag->type;
				if ($pricetag->promoprice != $pricetag->price)
				  $_->r = money($pricetag->price);
				if ($discount > 0)
				  $_->d = $discount;
